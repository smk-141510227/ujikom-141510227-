...
@section('content1')
	...
	@elseif($data2->tunjangan_pegawai_id == $data1->id && $data1->pegawai->id != $data->id )
						...
						@else
					belum diambil
				<form class="form-horizontal" role="form" method="POST" action="{{ url('/penggajian') }}">{{ csrf_field() }}
		@foreach($tunjangan as $data1)
					@if($data1->pegawai_id == $data->id)
		<input type="hidden" name="tunjangan_pegawai_id" value="{{$data1->id}}">@php $a=$data1->tunjangan->besar_uang; ; @endphp
	@endif
					@endforeach
					@foreach($lemburp as $data1)
	@if($data1->pegawai_id == $data->id)
